...
def test_hamiltonian_monte_carlo():
    ...
    np.random.seed(1)
    neg_log_p = neg_log_normal(2, 0.1)
    samples = hamiltonian_monte_carlo(100, neg_log_p, np.array(0.))
    assert_allclose(2., np.mean(samples), atol=0.006)
...
def test_hamiltonian_monte_carlo_mv():
    ...
    samples = hamiltonian_monte_carlo(100, neg_log_p, np.zeros(mu.shape), path_len=2.)
